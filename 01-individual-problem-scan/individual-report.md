...
```mermaid
flowchart LR
  A["1· Mở đúng nhóm chat<br/>0.5p"] --> B["2· Search từ khóa<br/>thường trượt · 1p"]
  B --> C["3· Cuộn ngược thủ công<br/>2.5p"]
  C --> D["4· Đọc ngữ cảnh để xác nhận<br/>'đã chốt' hay 'đang bàn'<br/>3p"]
  D --> E["5· Không chắc thì hỏi lại nhóm<br/>1p + thời gian chờ"]
  class D bn
  classDef bn fill:#ffe3e3,stroke:#c92a2a,stroke-width:2px
```
...
```mermaid
flowchart LR
  A["1· Hỏi bằng câu tự nhiên<br/>0.5p · người"] --> B["2· AI tìm + tóm tắt quyết định<br/>kèm trích dẫn gốc · 0.5p"]
  B --> C["3· Người đọc trích dẫn gốc<br/>để tự xác nhận · 1p"]
  C --> D["4· Pin lại — CHỈ khi quan trọng<br/>+0.5p · Rule"]
  B -.->|"AI không tìm ra<br/>hoặc trích dẫn lệch"| F["FALLBACK<br/>cuộn tay như cũ + hỏi nhóm"]
  class B ai
  class C human
  class D rule
  class F fb
  classDef ai fill:#e7f5ff,stroke:#1971c2
  classDef human fill:#fff9db,stroke:#e67700,stroke-width:2px
  classDef rule fill:#ebfbee,stroke:#2f9e44
  classDef fb fill:#f8f9fa,stroke:#868e96,stroke-dasharray:4
```
...
```mermaid
flowchart LR
  A["1· Mở 4 app<br/>lấy lịch sử 30 ngày · 8p"] --> B["2· Chép sang file tổng<br/>7p"]
  B --> C["3· Gộp giao dịch trùng<br/>giữa 2 app của mình · 5p"]
  C --> D["4· Phân loại ~70 giao dịch<br/>mô tả viết tắt, mỗi app một kiểu<br/>12p"]
  D --> E["5· Cộng + so tháng trước<br/>3p"]
  class D bn
  classDef bn fill:#ffe3e3,stroke:#c92a2a,stroke-width:2px
```
...
```mermaid
flowchart LR
  A["1· Xuất/chụp lịch sử 4 app<br/>4p · vẫn thủ công, app không mở API"] --> B["2· Rule khớp từ khóa<br/>merchant lớn ~70% · 0.5p"]
  B --> C["3· AI phân loại ~30% còn lại<br/>mô tả tự do · 0.5p"]
  C --> D["4· Tôi review nhãn AI gán<br/>+ mọi giao dịch trên 500k · 3p"]
  D --> E["5· Xem tổng + cảnh báo khoản lạ<br/>1p"]
  C -.->|"AI gán nhãn sai nhiều"| F["FALLBACK<br/>tắt bước 3, chỉ dùng Rule<br/>phần còn lại phân loại tay"]
  class B rule
  class C ai
  class D human
  class F fb
...
```mermaid
flowchart LR
  A["1· Search trên 2-3 sàn<br/>4p"] --> B["2· Mở ~4 tab so giá<br/>+ phí ship + KM · 6p"]
  B --> C["3· Đọc ~15 review<br/>9p"]
  C --> D["4· Cân review mâu thuẫn<br/>đoán review nào thật · 4p"]
  D --> E["5· Chốt shop và đặt<br/>2p"]
  class D bn
  classDef bn fill:#ffe3e3,stroke:#c92a2a,stroke-width:2px
```
...
```mermaid
flowchart LR
  A["1· Nhập món cần mua<br/>+ ngân sách · 1p"] --> B["2· Rule lọc cứng<br/>giá trần, shop mall, trên 4.5 sao · 0.5p"]
  B --> C["3· AI tóm tắt review 3 shop<br/>nêu rõ ĐIỂM MÂU THUẪN · 1.5p"]
  C --> D["4· Tôi đọc tóm tắt<br/>+ mở 1-2 review gốc để kiểm · 4p"]
  D --> E["5· Tự chốt và đặt<br/>2p"]
  C -.->|"AI tóm tắt sai<br/>hoặc bịa review"| F["FALLBACK<br/>đọc tay như cũ<br/>luôn giữ link review gốc"]
  class B rule
  class C ai
  class D human
  class F fb
