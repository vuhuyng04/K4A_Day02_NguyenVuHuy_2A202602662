...
```mermaid
flowchart TB
  S1["Bước 1-3 · Xác định câu hỏi → tạo nhóm từ khóa → search Scholar<br/>Researcher · lặp nhiều vòng"]
  S2["Bước 4-6 · Mở publisher/arXiv/venue → đọc và loại trùng → lần references/cited-by<br/>Researcher · handoff giữa 4 nguồn"]
  S3["Bước 7-8 · Lập literature matrix → tổng hợp limitations<br/>Researcher · chưa có time-log, dễ mất provenance"]
  S4["Bước 9-10 · Đề xuất candidate gap → search phản chứng<br/>Researcher"]
  S5["Bước 11-12 · Đánh giá data/compute/time/skill → chọn gap + kiểm citation<br/>Researcher + giảng viên"]
  S1 --> S2 --> S3 --> S4 --> S5
  class S4 bn
...
```mermaid
flowchart TB
  A["1· Nhập question + phạm vi + resource profile"]
  B["2· AI tạo taxonomy và query"]
  C["3· Search API lấy candidate papers<br/>Semantic Scholar · Crossref"]
  D["4· Rule xác minh DOI, metadata, duplicate, retraction"]
  E["5· AI trích task, dataset, method, metric, limitation<br/>kèm đoạn nguồn"]
  F["6· Người dùng duyệt inclusion"]
  G["7· AI dựng evidence map và candidate gaps"]
  H["8· Search phản chứng từng gap"]
  I["9· Rule chấm feasibility theo data/compute/time/skill"]
  J["10· Người dùng + giảng viên chọn gap"]
  K["11· Xuất matrix, gap brief và citation"]
  A --> B --> C --> D --> E --> F --> G --> H --> I --> J --> K
  D -.->|"thiếu full text hoặc<br/>metadata mâu thuẫn"| X["FALLBACK<br/>gắn nhãn 'chưa xác minh'"]
  H -.->|"gap thiếu 2 nguồn hỗ trợ<br/>hoặc chưa counter-search"| Y["FALLBACK<br/>loại khỏi shortlist"]
  class B ai
  class E ai
  class G ai
  class D rule
